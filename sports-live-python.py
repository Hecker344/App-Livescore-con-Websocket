...
class Match:
    """Classe per gestire un singolo match"""
    def __init__(self, match_id, sport, home_team, away_team, status='scheduled'):
        ...
        if sport == 'basketball':
            self.max_time = 48
            self.quarter = 1 if status == 'scheduled' else random.randint(2, 3)
            self.time = 0 if status == 'scheduled' else random.randint(5, 15)
    ...
    def update(self):
        ...
        if self.sport == 'basketball':
            ...
            # Cambio quarto
            if self.time % 12 == 0 and self.time < self.max_time:
                self.quarter += 1
                event = {
                    'id': len(self.events) + 1,
                    'minute': self.time,
            ...
            # Ippon - vittoria immediata
            if not self.ippon and random.random() < 0.015:  # 1.5% probabilità ippon
                team = random.choice(['home', 'away'])
                self.ippon = True
                
                if team == 'home':
                    self.home_score = 10  # Ippon = vittoria
                else:
                    self.away_score = 10
                ...
                event = {
                    ...
                    'type': 'ippon',
                    'team': team,
                    'technique': random.choice(['Seoi-nage', 'Uchi-mata', 'O-soto-gari', 'Harai-goshi', 'Tai-otoshi'])
                }
                self.events.append(event)
                self.status = 'finished'
            
            # Wazari
            elif random.random() < 0.025:  # 2.5% probabilità wazari
                team = random.choice(['home', 'away'])
                
                if team == 'home':
                    self.wazari_home += 1
                    if self.wazari_home >= 2:  # 2 wazari = ippon
                        self.home_score = 10
                        self.status = 'finished'
                else:
                    self.wazari_away += 1
                ...
                event = {
                    ...
                    'type': 'wazari',
                    'team': team,
                    'technique': random.choice(['Seoi-nage', 'Ko-uchi-gari', 'Osoto-gari', 'Uchi-mata'])
                }
                self.events.append(event)
...
def initialize_matches():
    ...
    teams = {
        'basketball': ['Lakers', 'Celtics', 'Warriors', 'Heat', 'Nets', 'Bulls', 'Bucks', 'Suns'],
        'judo': ['Teddy Riner', 'Ono Shohei', 'Lukasz Krpalek', 'Ilias Iliadis', 
                 'Takato Naohisa', 'Daria Bilodid', 'Clarisse Agbegnenou', 'Nora Gjakova'],
        'cricket': ['India', 'Australia', 'England', 'Pakistan', 'New Zealand', 'South Africa']
    }
    
    matches_data = [
        ('1', 'basketball', 'Lakers', 'Celtics', 'live'),
        ('2', 'judo', 'Teddy Riner', 'Ono Shohei', 'live'),
        ('3', 'cricket', 'India', 'Australia', 'live'),
        ('4', 'basketball', 'Warriors', 'Heat', 'finished'),
        ('5', 'cricket', 'England', 'Pakistan', 'scheduled'),
    ]
